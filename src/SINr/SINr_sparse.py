...
@numba.njit(parallel=True)
def _arrays_SINr(edges, vector, size_partition, nb_nodes):
...
@numba.njit(parallel=True)
def _compute_SINr(sum_degrees_com, vector, size_partition,weighted,indptr,indices,data,positions):
...
def get_SINr_embeddings(edges, weights, vector, size_partition, nodes):
    ...
    sum_degrees_com,indptr,indices,data=_arrays_SINr(numba_edges, vector, size_partition, nodes)
    int_degree=csr_matrix((data,(indptr,indices)), shape=(nodes,size_partition))
    positions=[(i,j) for i, j in zip(*int_degree.nonzero())]

    np_indptr, np_indices, np_data, nr_indptr, nr_indices, nr_data = _compute_SINr(sum_degrees_com, vector, size_partition, numba_weighted, int_degree.indptr,int_degree.indices,int_degree.data,positions)
